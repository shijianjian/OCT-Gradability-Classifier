...
class SE_ResNeXt(object):
    ...
    def transform_layer(self, x, strides, scope, type='bottleneck'):
        with tf.name_scope(scope):
            x = Conv3D(filters=self.depth, kernel_size=(1, 1, 1) if type == "bottleneck" else (3, 3, 3), strides=(1, 1, 1), padding='same')(x)
    ...
    def residual_layer(self, input_x, out_dim, layer_num, res_block=None):
        ...
        for i in range(res_block):
            ...
            if self.cardinality == 1:
                x = self.transform_layer(input_x, strides=strides, scope='basic', type='basic')
            else:
...
if __name__ == '__main__':
    model = SE_ResNeXt(cardinality=8, blocks=3, depth=32, reduction_ratio=4, init_filters=32,
                       training=True).build(input_shape=(200, 1024, 200, 1), num_output=2, repetitions=6)
    print(model.summary())
